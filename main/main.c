...
static void bleprph_advertise(void)
{
     ...
     */
    fields.tx_pwr_lvl_is_present = 0;  //1 changed
    ...
    adv_params.itvl_min = (CONFIG_EXAMPLE_ADVERTISE_INTERVAL*100/625);
    adv_params.itvl_max = (CONFIG_EXAMPLE_ADVERTISE_INTERVAL*1000/625);

    rc = ble_gap_adv_start(own_addr_type, NULL, 10*1000,&adv_params, bleprph_gap_event, NULL);
    if (rc != 0) {
 ...
 *                                  particular GAP event being signaled.
 */
static int bleprph_gap_event(struct ble_gap_event *event, void *arg)
{
...
void BLE_init(void)
{
   ...
   /* Initialize NVS — it is used to store PHY calibration data */
    ...
    rc = ble_svc_gap_device_name_set("ICT_LAB_YOUSUF");
    assert(rc == 0);
